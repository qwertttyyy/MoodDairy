...
Приложение задумано как инструмент самонаблюдения и помощи в работе с психиатром или психотерапевтом.
...
| POST | `/` | Создать ссылку (деактивирует предыдущую) |
...
Переключение: `ENCRYPTION_ENABLED=0` в `.env` отключает шифрование (клиент отправляет данные в открытом виде).
